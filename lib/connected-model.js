...
module.exports = function(baseRoute, model, modelName, options) {
    ...
    var routes = {};
    var middlewareByRoute = {POST: {}, GET: {}};
    ...
    var fixParams = function(req, fn, params) {
        ...
        if ( !!fn.inputTransformer ) {
            params = fn.inputTransformer(req, params);
        }
    ...
    connectedModelStaticFunctions.forEach(function(functionName) {
        ...
        middlewareByRoute.GET[route + '(/?).*'] = function(req, res, next) {
            var params = fixParams(req, model[functionName], getParamsFromUrl(req.url, route));

            return model[functionName].apply(null, params).then(function(result) {
    ...
    connectedModelInstanceFunctions.forEach(function(functionName) { 
        ...
        middlewareByRoute.POST[route] = function(req, res, next) {
            ...
            P.all(targets.map(function(target) { 
                target = utils.classify(target, model);
                return model.prototype[functionName].apply(target, params);
        ...
        renderer.render(__dirname + '/../views/connected_model.jade', 
          {
             ...
             additionalIncludes: options.additionalIncludes || {},
    ...
    if ( options.exposeDynamicJsRoute ) {
        middlewareByRoute.GET[baseRoute + '/model.js'] = function(req, res) {
    ...
    return {
        middleware: function(req, res, next) {
            var routesForMethod = middlewareByRoute[req.method],
                invokeMatchingRoute = function(route) {
                    var regex = new RegExp('^' + route + '$', 'i');
                    if ( regex.test(req.url) ) {
                        routesForMethod[route](req, res, next);
                        return true;
                    }
            ...
            if ( Object.keys(routesForMethod).some(invokeMatchingRoute) ) {
                return;
            }
