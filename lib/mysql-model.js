    ...
    var getDb = function(context, cb) {
        ...
        function releaseDb(error, db) {
            ...
            if ( !!error && !!factory ) {
                ...
                return factory(function(error, newDb) {
                    if ( !!error ) {
                        log.error('Error creating replacement db connection');

                        // leave what we have for now
        ...
        if ( !!context && !!context._dbConnection ) {
            ...
            pool.acquire(function(err, db) {
                ...
                deferred.success(releaseDb.bind(null, undefined, db)).fail(function() {
                    log.error('Error executing db routine', arguments);

                    return releaseDb(db, true);
                });
