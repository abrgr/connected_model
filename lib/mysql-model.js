...
var MySqlModel = module.exports = function(model, pool, options) {
    ...
    var localNSFieldsByAlias = _.extend.apply(_, getAliasedAndNamespacedFields(tableName, allFields));
    ...
    var localAliasedFieldNamesByPropertyName = _.extend.apply(_, Object.keys(options.fields).map(function(propertyName) {
            var ret = {};
    ...
    var fieldNamesByPropertyName = _.extend.apply(_, Object.keys(options.fields).map(function(propertyName) {
            var ret = {};
    ...
    var getFieldValues = model._getFieldValues = function(obj) {
        return _.flatten(_.compact(Object.keys(recursiveNSFieldsByAlias).map(function(alias) {
            var property = recursivePropertyNamesByAliasedFieldName[alias];
            if ( _.isUndefined(obj[property]) ) {
                return null;
            }

            if ( options.fields[property] && options.fields[property].join ) {
                // return a list of {field, fieldValue} objs.  Will be compacted
                return options.fields[property].join._getFieldValues(obj);
            }
    ...
    model._toModel = function(db, rows) {
        var rowsById = _.groupBy(rows, idFields[0]);
