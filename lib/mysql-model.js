...
var MySqlModel = module.exports = function(model, pool, options) {
        ...
        Object.keys(row).forEach(function(alias) {
            ...
            if ( _.isUndefined(propName) ) {
                ...
                if ( matchingJoinProps.length > 1 ) {
                    log.error('Join property not met', fieldName, propertyNamesByFieldName, row, fieldTableName, model);
                }
    ...
    model.match = function(match, context) {
        ...
        return getDb(context, function(db, deferred) {
            ...
            if ( match.$sortAscending ) {
                var ascOrder = {};
                ascOrder[options.fields[match.$sortAscending].field] = true;
                query.order(ascOrder);
            }

            if ( !!match.$sortDescending ) {
                var descOrder = {};
                descOrder[options.fields[match.$sortDescending].field] = false;
                query.order(descOrder);
            }
