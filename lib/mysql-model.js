...
var MySqlModel = module.exports = function(model, pool, options) {
        ...
        return getFields(true, thisTableOnly).filter(function(fieldName){return !!obj[propertyNamesByFieldName[fieldName]];})
            ...
            if ( !!field.join ) {
                return field.join._idProperties.map(function(joinIdProperty) {
                    return {field: field.field, fieldValue: obj[key][joinIdProperty]};
                });
            }
    ...
    var fillInManyToManies = model._fillInManyToManies = function(db, obj) {
        ...
        var intermediateDeferreds = [];
        manyToManyProperties.forEach(function(manyToManyProperty) {
            ...
            */
            ...
            intermediateQuery.execute(function(error, rows, cols) {
                ...
                return rows.forEach(function(rows) {
                    var matchObj = {};
                    Object.keys(manyToMany.thatFields).forEach(function(prop) {
                        ...
                        manyToMany.thatModel.match(matchObj).success(function(results) {
                            d.resolve({property: manyToManyProperty, values: results});
                        }).fail(d.rejected.bind(d));
                    });
                    manyToMany.thatModel.match();
                });
            });
        });

        Deferred.afterAll(intermediateDeferreds).success(function(results) {
            results.forEach(function(result) {
                var matchResults = result[0];
                obj[matchResults.property] = (obj[matchResults.property] || []).concat(matchResults.values);
            });
    ...
    var toModel = model._toModel = function(db, rows) {
		rows.map(console.log.bind(console, 'bound row'));
		var d = new Deferred();
