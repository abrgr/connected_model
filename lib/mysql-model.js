...
var MySqlModel = module.exports = function(model, pool, options) {
    ...
    var toSqlValue = function(val) {
        if ( !_.isUndefined(val) && _.isFunction(val._toSqlValue) ) {
    ...
    var getDb = function(context, cb) {
        ...
        function releaseDb(db) {
            if ( !!context && !!context._inTxn ) {
                console.log('Not releasing connection--in txn');
                return;
            }
        ...
        if ( !!context && !!context._dbConnection ) {
            ...
            console.log('Acquiring new connection');
            pool.acquire(function(err, db) {
                ...
                deferred.success(releaseDb.bind(null, db)).fail(releaseDb.bind(null, db));
            ...
            if ( 'object' === typeof(fieldValue) ) {
                ...
                return Object.keys(sqlForSpecialMatchConditions).forEach(function(matchCondition) {
                    ...
                    if ( !!matchConditionValue ) {
                        if ( matchCondition === '$isNull' ) { // TODO: pretty hackish.  put this in some nicer config for 0-arg conditions
    ...
    model.get_by_id = function(ids, context) {
        ...
        return this.matchSingle(ids, context);
    };
    ...
    model.prototype.createTransaction = function(name, fn) {
        ...
        pool.acquire(function(err, db) {
            ...
            console.log('Acquired db connection for transaction', name);
            ...
            function endTxn(db, commit) {
                if ( commit ) {
                    db.query().execute('COMMIT', function(err, result) {
                        pool.release(db);
                        if ( !!err ) {
                            console.log('Failed to commit transaction', name);
                            return deferred.reject(err);
                        }

                        console.log('Committed transaction', name);
                        return deferred.resolve(Array.prototype.slice.call(arguments, 2));
                    });
                }
            }
            ...
            db.query().execute('START TRANSACTION', function(err, result) {
                if ( !!err ) {
                    console.log('Failed to start transaction', name);
                    return deferred.reject(err);
                }

                console.log('Started transaction', name);

                return deferred.guard(txn, fn, deferred); 
            });
        });
