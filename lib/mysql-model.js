...
var MySqlModel = module.exports = function(model, pool, options) {
    ...
    function getComputedFieldValues(obj) {
        return _.map(computedFields, function(descriptor, prop) {
            return {
                field: namespaceField(tableName, descriptor.field),
                fieldValue: toSqlValue(obj[prop]() || null)
            };
        });
    }

    function getUpdateTimestampFieldValues(obj) {
        var fieldValues = Object.keys(localNSFieldsByAlias).map(function(alias) {
            var property = localPropertyNamesByAliasedFieldName[alias];
            if ( property !== updateTimestampProperty ) {
                return null;
            }

            return {
                field: recursiveNSFieldsByAlias[alias],
                fieldValue: toSqlValue(obj[updateTimestampProperty] || new Date())
            };
        });

        return _.compact(fieldValues);
    }
     ...
     * a {field, fieldValue} for the updateTimestampProperty, set to now, as well as fill
     * in any computed fields.
     **/
    var getFieldValues = model._getFieldValues = function(obj, forUpdate) {
        // fill in computed fields
        var computedFieldValues = forUpdate
                                ? getComputedFieldValues(obj)
                                : [];
        // fill in update timestamp
        var updateTimestampFieldValues = forUpdate
                                       ? getUpdateTimestampFieldValues(obj)
                                       : [];

        var forUpdateFieldValues = computedFieldValues.concat(updateTimestampFieldValues);
        ...
        var ret = _.flatten(_.compact(Object.keys(localNSFieldsByAlias).map(function(alias) {
            ...
            if ( forUpdate && property === updateTimestampProperty ) {
                // we already handled the update timestamp
                return null;
            }
    ...
    var addOrderBys = model._addOrderBys = function(db, query, match) {
        ...
        var fieldValues = getFieldValues(match, false);
    ...
    model.prototype.insert = function(userDefinedIds) {
        ...
        if ( _.isFunction(self.isValid) && !self.isValid() ) {
            return P.reject(new Error('model is not valid'));
        }

        var passedInIds = getIds(self);
        if ( !userDefinedIds && !!passedInIds ) {
            log.warn('insert called on a model that already has an id', self);
        }
    ...
    model.prototype._reserveId = function() {
        ...
        var BS_UNIQUE_DELETE_MARKER = 666;
        ...
        return getDb(self, function(db) {
            ...
            function reserveId() {
                ...
                idReserver[deleteMarkerProperty] = BS_UNIQUE_DELETE_MARKER;
                var txnContext = {
                    _dbConnection: db,
                    _inTxn: true,
                    _txnName: 'reserveid'
            ...
            var idPromise;
            if ( ctx._inTxn ) {
                // we'll use a savepoint if we're already in a transacation
                idPromise = ninvoke(db.query(), 'execute', 'SAVEPOINT RESERVEID').then(reserveId).then(function(result) {
                    var id = result.id;
                    return ninvoke(db.query(), 'execute', 'ROLLBACK TO SAVEPOINT RESERVEID').return(id);
                });
            } else {
                // we'll create a transaction
                idPromise = ninvoke(db.query(), 'execute', 'START TRANSACTION').then(reserveId).then(function(result) {
                    var id = result.id;
            ...
            return idPromise.then(function(id) {
                if ( (+id) === BS_UNIQUE_DELETE_MARKER ) {
                    log.warn('Re-trying reserveId since our id is the delete marker', id);
                    return self._reserveId();
                }
        ...
        return getDb(self, function(db) {
            ...
            if ( deleteMarkerProperty ) {
                ...
                return self._reserveId(self).then(function(deleteId) {
                    ...
                    // we don't pass forUpdate to the fieldValues because then we would need to pass an actual model object
                    var deleteFieldsAndNames = getFieldValues(deleteSetter).concat(getComputedFieldValues(self)).concat(getUpdateTimestampFieldValues(self));
                    var fieldsToUpdate = _.pluck(deleteFieldsAndNames, 'field');
                    var fieldValuesToUpdate = _.pluck(deleteFieldsAndNames, 'fieldValue');

                    return executeQueryPart(db.query().update(tableName).set(utils.objectZip(fieldsToUpdate, fieldValuesToUpdate)));
                });
            } else {
