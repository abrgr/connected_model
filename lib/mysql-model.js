...
var MySqlModel = module.exports = function(model, pool, options) {
    ...
    var idProperty = model._idProperty = Object.keys(options.fields)
                        .filter(function(key) { return options.fields[key].id; })[0];
    var idField = model._idField = options.fields[idProperty].field;
    ...
    var getFieldValues = function(obj, thisTableOnly) {
        ...
        return getFields(true, thisTableOnly).filter(function(fieldName){return !!obj[propertyNamesByFieldName[fieldName]];})
            ...
            if ( !!field.join ) {
                return {field: field.field, fieldValue: obj[key][field.join._idProperty]};
            }
    ...
    var getId = function(model) {
        var passedInId = model[idProperty];
    ...
    model.get_by_id = function(id) {
        ...
        return getDb(null, function(db, deferred) {
            ...
            addJoins(query).where(idField + '=?', [id]).execute(function(error, rows, cols) {
                ...
                return deferred.resolve(singleRowToModel(rows[0], model));
            });
        });
    };
    ...
    model.prototype.update = function() {
        ...
        return getDb(this, function(db, deferred) {
            ...
            var requiredFieldValues = [passedInId].concat(requiredJoinFields.map(function(field){return self[field];}));
            var fieldsAndValues = getFieldValues(self, true);
            var fieldNames = fieldsAndValues.map(function(fieldValue) { return fieldValue.field; });
            var fieldValues = fieldsAndValues.map(function(fieldValue) { return fieldValue.fieldValue; });
