...
var MySqlModel = module.exports = function(model, pool, options) {
    ...
    var idField = model._idField = Object.keys(options.fields).map(function(key) { return options.fields[key]; })
    ...
    var getJoins = function() { 
        return Object.keys(options.fields).filter(function(key) { return !!options.fields[key].join; }).map(function(key) {
            ...
            return {
                ...
                conditions: tableName + '.' + j.field + '=' + j.join._tableName + '.' + j.join._idField
            };
        });
    };

    var addJoins = function(query) {
        var joins = getJoins();
        if ( joins.length > 0 ) {
    ...
    var getFields = model._getFields = function(skipNamespaces) {
    ...
    var getFieldValues = function(obj) {
        return getFields(true).filter(function(fieldName){return !!obj[propertyNamesByFieldName[fieldName]];})
                          .map(function(fieldName){
            var key = propertyNamesByFieldName[fieldName];
            var field = options.fields[key];
            if ( !!field.join ) {
                return {field: field.field, fieldValue: obj[key][field.join._idField]};
            }

            return {field: field.field, fieldValue: obj[key]};
        });
    };
    ...
    var addWheres = function(db, query, match) {
    ...
    var singleRowToModel = model._singleRowToModel = function(row) {
        ...
        Object.keys(row).forEach(function(fieldName) {
            ...
            if ( !!field.join ) {
                modelRow[propName] = field.join._singleRowToModel(row);
            } else {
    ...
    var toModel = function(rows) {
        return rows.map(singleRowToModel);
    };
    ...
    model.get_by_id = function(id) {
        ...
        return getDb(null, function(db, deferred) {
            ...
            addJoins(query).where(idField + '=?', [id]).execute(function(error, rows, cols) {
                pool.release(db);
                ...
                return deferred.resolve(singlRowToModel(rows[0], model));
            });
        });
    };
    ...
    model.match = function(match) {
        ...
        return getDb(this, function(db, deferred) {
            ...
            query.execute(function(error, rows, cols) {
                ...
                return deferred.resolve(toModel(rows));
            });
        });
    };
    ...
    model.prototype.insert = function() {
        ...
        return getDb(this, function(db, deferred) {
            var self = this;
            var fieldsAndValues = getFieldValues(self);
    ...
    model.prototype.update = function() {
        ...
        return getDb(this, function(db, deferred) {
            var self = this;
            var requiredFieldNames = [idField].concat(requiredJoinFields);
            var requriedFieldValues = [passedInId].concat(requiredJoinFields.map(function(field){return self[field];}));
            var fieldsAndValues = getFieldValues(self);
            var fieldNames = fieldsAndValues.map(function(fieldValue) { return fieldValue.field; });
            var fieldValues = fieldsAndValues.map(function(fieldValue) { return fieldValue.fieldValue; });
            db.query().update(tableName).set(utils.objectZip(fieldNames, fieldValues))
    ...
    model.prototype.delete = function() {
        ...
        return getDb(this, function(db, deferred) {
            var self = this;
            var fieldNames = [idField].concat(requiredJoinFields);
            var fieldValues = [passedInId].concat(requiredJoinFields.map(function(field){return self[field];}));
